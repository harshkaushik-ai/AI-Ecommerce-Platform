...
const LoginModal = () => {
  ...
  const [mode, setMode] = useState("signin")  //Signin || Signup || reset || forgot
  ...
  <div className="fixed inset-0  z-50 flex items-center justify-center">
    ...
    <div className="relative z-10 glass-panel w-full max-w-md mx-4 animate-fade-in-up">
      ...
      <form onSubmit={handleSubmit} className=" space-y-4">
        ...
        {/* EMAIL ALWAYS VISIBLE EXCEPT RESET  */}
        {mode !== "reset" && (
            <div className="relative">
        ...
        {mode !== "forgot" && (
            <div className="relative">
              ...
              <span className="absolute right-3 top-1/2 transform -translate-y-1/2">
                  <button className="text-xs text-muted-foreground flex items-center gap-1" type="button">
                    {
                      showPassword ? (
                        <EyeOff className="w-4 h-4 text-primary" onClick={()=>setShowPassword(!showPassword)}/>
                      ):(<Eye className="w-4 h-4 text-primary " onClick={()=>setShowPassword(!showPassword)}/>)
                    }
        ...
        {/* CONFIRMPASSWORD ONLY VISIBLE FOR RESET */}
        {mode === "reset" && (
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-muted-foreground"/>
              <input 
        ...
        >
          {isLoading ? (
            ...
            <div className="w-5 h-5  border-2 border-white border-t-transparent rounded-full animate-spin"/>
              <span>
                {
                  mode === "reset" ? "Reseting password..." : mode === "signup" ? "Signing up..." : mode === "forgot" ? "Requesting for email..." : "Signing in..."}{" "}
              </span>
